...
- **Processing**: The backend sends the captured conversation to Google Gemini to create the Context Carry summary. If Gemini fails, it may send the conversation through OrcaRouter's free route to its selected upstream model, then use Mistral or Groq as later fallback summarization providers.
...
- **Transfer Analytics**: Each transfer attempt sends metadata to our Supabase analytics system: a random extension-install identifier, a random attempt identifier, timestamp, source and destination platform, captured character count when known, success or failure status, the last predefined pipeline stage reached, a predefined non-sensitive failure category, and extension version. The closed stages cover intent, capture, summary request/response/completion, paste, and completion. Telemetry never includes chat text, generated summaries, page URLs, stack traces, arbitrary error messages, or provider response bodies. The random install identifier is created once in local extension storage and is not tied to a Cap Context account, name, email address, or AI account. After an install's first successful summary, one protected internal `users` row assigns it a sequential number and tracks only its lifetime successful-summary count, current UTC-day successful-summary count, and the UTC date for that daily count. This does not create a user account or identify the person, and the aggregate row contains no per-transfer activity or conversation content.
- **User Cancellation**: If the source AI tab is closed while a transfer is active, telemetry may classify the attempt as `user_cancelled`. This records only the safe category and last pipeline stage, not what the user was doing or any conversation content.
