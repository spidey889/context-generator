...
- Claude: fixed to the stable page root but measured beside a composer-local Voice/Send/mic anchor. A surface is accepted only when it contains the editor and an eligible visible control whose geometry fits that surface; null-anchor states never fall back to a synthetic bottom-right position. The last anchored viewport position survives a bounded 700 ms hydration/remount gap. SPA pathname changes force fresh route-specific alignment. The docked `/chat/...` optical adjustment is applied after the legacy composer-local bound so a 48 px surface cannot erase it; final placement remains viewport-clamped. Reserve the Voice/Send switch through its persistent compact grid cluster, which contains both Claude state branches, using the independent CSS `translate` property. New controls then inherit the reserved position before they mount, and Claude owns all child layout, transforms, transitions, and intermediate editor/control commits. Individual-button translation is only a bounded fallback when the stable cluster is unavailable during hydration.
...
- Gemini: left of the Pro/Flash selector; retains the outer composer during large-paste expansion. A replacement composer, anchor, fallback mode, or viewport position must remain unchanged for three animation frames before Gemini reparents or moves the orb; until then the last confirmed viewport position remains fixed.
- Grok: beside whichever visible Fast, Build, Auto, Expert, Heavy, or thinking-mode selector is active; retains the outer composer and requires focused paste. Like Gemini, Grok confirms a changed composer, anchor/fallback mode, and viewport position across three animation frames before committing it, so a partially remounted control row cannot displace the last known-good position.
- DeepSeek: near attachment/input controls; retains the outer composer during expansion. DeepSeek also requires a changed composer, complete-row anchor/fallback mode, and viewport position to remain unchanged for three animation frames before replacing its last confirmed placement.
...
- Any content-script change: update `CONTENT_SCRIPT_LOAD_ID` so open tabs replace stale code, and retain stale-node/reservation cleanup. Current value: `platform-content-2026-09-25-summary-paste-v34`.
