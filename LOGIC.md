...
The default model is `ministral-3b-2512`, with `MISTRAL_MODEL` available as an environment override. Temperature is `0.1`, and output is capped at `650` tokens by default through `MISTRAL_MAX_TOKENS` so long generations do not make the handoff feel stuck. The endpoint returns timing metadata with the summary, including total backend time, Mistral time, model, max tokens, input characters, and output characters.
