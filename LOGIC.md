...
The summarizer model is fixed to `mistral-large-2512`. Temperature is `0.1`, and output is capped at `4200` tokens by default through `MISTRAL_MAX_TOKENS` so the Context Carry block has enough room to reach the 1200-word target. The prompt and required template now target about 1200 useful words for substantial conversations, with explicit section budgets and instructions to preserve concrete files, functions, commands, errors, tests, deployment state, user constraints, unresolved risks, validation gaps, and uncertainty instead of a thin executive summary. If a substantial conversation produces a normalized summary under 1100 words, the backend makes a second Mistral expansion pass using the original conversation and the draft summary, then uses the expanded version only if it is longer. The per-call Mistral request timeout is 80 seconds and the extension backend timeout is 150 seconds so the expansion pass has room to finish instead of being cut off by the old speed-focused limits. `vercel.json` sets `api/summarize.js` `maxDuration` to 180 seconds. The endpoint returns timing metadata with the summary, including total backend time, Mistral time, model, max tokens, target words, minimum words, summary word count, Mistral pass count, expansion metadata, input characters, and output characters. The extension/background trace also logs `inputChars`, `requestChars`, and `backendInputChars` so payload size can be checked from console timing.
