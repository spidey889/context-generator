...
The root `index.html` is a deliberately minimal, dependency-free marketing page with three parts: a focused hero and static handoff cue, a three-step explanation, and one control/privacy section with the final install action. It has no JavaScript, build step, runtime API dependency, carousel, FAQ, animated demo, repeated proof metrics, or repeated install walkthrough. It uses relative assets and respects reduced motion. Its public install path is the Chrome Web Store for Chrome and Brave. The static handoff cue is illustration only and never captures a conversation or starts a transfer.
