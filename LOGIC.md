...
- Mistral is skipped without `MISTRAL_API_KEY`. The chain is exactly Mistral Large 3 (55-second budget) then Ministral 3 14B (25-second budget). Rate limits are model-specific, so HTTP 429 advances immediately instead of retrying the same model.
