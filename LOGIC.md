...
Placement is platform-specific: Claude anchors beside voice controls, ChatGPT near its model selector at the page root, Gemini immediately left of its in-composer `Pro` or `Flash` model selector, Grok near its mode selector, and DeepSeek near attachment. Gemini, Grok, and DeepSeek accept expanded composers up to 720px tall. During staged reflow after a large paste, each retains its already-verified outer composer through temporary input/container geometry mismatch and observes its input/composer resizing so placement follows the controls before paint; the shared 260px composer-surface limit and mutation-only monitoring remain unchanged for Claude and ChatGPT. Bounded editor/composer fallbacks handle hydration or selector drift; they are not conversation-capture fallbacks. Extension-owned nudges are excluded from capture.
