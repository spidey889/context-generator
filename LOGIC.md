...
- `supabase/migrations/`: production database changes, including keeping telemetry unavailable through the anonymous Data API and providing protected internal user-activity views.
...
Each distinct install id is also mapped automatically to a sequential internal label such as `User 1`; this is an anonymous installation label, not a login, account, name, or other identity. The protected `user_daily_usage` view omits install ids and groups transfer attempts by User N and UTC date, showing only the daily attempt count. The protected `user_transfer_activity` view joins that label to individual transfer events, so one row shows one attempt's timestamp, route, character count, status, and safe failure reason. Neither view exposes install ids or conversation content, and there are no first-seen or last-seen profile fields.
...
Placement is platform-specific: Claude anchors beside voice controls, ChatGPT immediately left of its live `Instant`, `Medium`, or `High` model selector, Gemini immediately left of its in-composer `Pro` or `Flash` model selector, Grok near its mode selector, and DeepSeek near attachment. All platforms accept expanded composers up to 720px tall. During staged reflow after a large paste, Claude, Gemini, Grok, and DeepSeek retain their already-verified outer composer while observing input/composer resizing. ChatGPT preserves its distinct page-root fixed-position architecture: it retains only the last verified placement surface and observes that surface plus the input, without mounting into or reserving the composer. ChatGPT model-selector discovery accepts only real `button` elements that share the editor's form or verified composer surface and satisfy the bounded control-row geometry; page-wide text, spans, tabindex wrappers, and streaming-response content are never eligible anchors. Bounded editor/composer fallbacks handle hydration or selector drift; they are not conversation-capture fallbacks. Temporary ChatGPT-only trigger diagnostics currently identify every scheduled, coalesced, skipped, and executed placement update; record child-list and resize-observer callbacks; and separately record paste/input, click, scroll, transition, attribute, and character-data signals that do not schedule updates. The diagnostics do not add any placement trigger or change placement behavior. Extension-owned nudges are excluded from capture.
