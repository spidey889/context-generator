...
3. With at least one verified turn present, the source shows the compact centered handoff popup with the same short entrance treatment and opens the chosen destination inactive while capture continues. A pointer-transparent neutral 9% black scrim sits behind the card to separate it consistently from light and dark source pages without changing page interaction. The card now follows the supplied Willow reference palette: a saturated violet-to-periwinkle base, deep purple lower depth, warm cream and muted rose light at the upper left, and frosted blue light at the upper right. Those cloudy fields stay concentrated across the top, drift slowly using transform/opacity-only motion, and shift subtly from left to middle to right with the real capture, summary, and paste stages; reduced-motion users receive the same static treatment. The Cap Context icon is rendered pearl-white for contrast, while the connector, live progress head, stage markers, countdown, inner rim, and layered shadow use coordinated lavender, frost, and deep-violet tones without changing the card geometry. A 4px progress head rides the exact live connector position while a stage is active and disappears when that stage completes. The card keeps the bubble icon, `Cap Context` label, and fixed 30-second countdown in one vertically aligned header row. When that countdown expires, the timer disappears and the same top-right header slot shows `Almost done, don't cancel now` until the handoff closes. The reassurance has no pill or background and uses a slightly larger, high-contrast whitish version of the destination sheet's Georgia face; it does not add a second timer or affect transfer work. The larger centered live headline uses that Georgia display face as well. Only while the real summary stage is active, three normal white period characters sit directly on the headline baseline after `Summarizing` and hop sequentially on a relaxed 1.8-second cycle; they disappear on summary completion and become static when reduced motion is requested. Below it, the system-sans indicator has three deterministic stages: `Capturing chat`, `Summarizing`, and `Pasting into <destination>`. Pipeline start/done marks advance each stage between highlighted active, checked completed, and legible muted upcoming states. While capture is active, the first connector reads the scroll position and remaining distance already produced by the virtual sweep's diagnostics, then eases toward that real fraction without changing or awaiting scraper work. While summary is active, the second connector uses one 12-second linear CSS transition from 5% toward a 90% ceiling as display-only activity; it cannot claim completion, and the real `summary done` mark alone fills it to 100%. Headline changes still enter with a short event-driven fade/slide that respects reduced-motion preferences. During the real gap after summary completion and before the paste request starts, the headline says `Summary ready` and paste remains upcoming. No timer rotates or invents stage status copy, and no progress animation gates the pipeline.
4. Capture scrolls to the top, waits within bounded stability windows for delayed turns, expands safe reading controls, and then sends every chat through the same bounded rendered-window sweep. Each downward pixel advance is followed by an awaited two-sample render-stability window before the next advance; the settled rendered snapshot is reused for overlap, diagnostics, and exit decisions instead of rescanning the conversation. Newly mounted reading controls are checked once after each rendered window settles rather than during every stability sample. Rendered-message boundary anchoring is a fallback only when pixel scrolling cannot move. Steps use 60% of the viewport by default. A following step may use 90% only when the actual before/after rendered snapshots prove a sliding window with at least 50% ordered turn overlap and a real positional shift; unchanged, delayed, non-overlapping, and oversized-message windows remain at 60%. The sweep continues while scrolling moves or the rendered conversation changes, and a short chat exits naturally after the terminal quiet check finds nothing more to capture.
...
Conversation candidate containment is indexed once as a DOM parent-to-child tree. Wrapper classification and turn selection reuse ancestor/descendant lookups while preserving the existing containment rules, scoring, and output ordering.
...
`npm run eval` sends the versioned evaluation transcripts through `EVAL_ENDPOINT`, defaulting to production. A case that misses any quality or latency gate is retried once, and the stronger of the two results is evaluated so one transient provider response does not block every push. It fails when both attempts remain below 90% required-fact recall, contain a forbidden/incorrect fact, have invalid Context Carry structure, exceed 30 seconds for the small case or 60 seconds for the medium case, or when the selected case latencies total more than 90 seconds. Required facts may define explicit equivalent phrases so harmless paraphrases do not create false failures.
