...
- Claude: fixed to the stable page root but measured beside a composer-local Voice/Send/mic anchor. A surface is accepted only when it contains the editor and an eligible visible control whose geometry fits that surface; null-anchor states never fall back to a synthetic bottom-right position. The last anchored viewport position survives a bounded 700 ms hydration/remount gap. SPA pathname changes force fresh route-specific alignment. The docked `/chat/...` optical adjustment is applied after the legacy composer-local bound so a 48 px surface cannot erase it; final placement remains viewport-clamped. Voice/Send visibility changes and control remounts reserve the newly active control synchronously through the independent CSS `translate` property. Never override Claude's native `transform` or `transition`: its editor value and control DOM update in separate React commits, and its transition lifecycle clears outgoing control visuals.
...
- Any content-script change: update `CONTENT_SCRIPT_LOAD_ID` so open tabs replace stale code, and retain stale-node/reservation cleanup. Current value: `platform-content-2026-09-09-claude-independent-translate-v13`.
