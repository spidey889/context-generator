...
The summarizer model is fixed to `mistral-large-2512`. Temperature is `0.1`, and output is capped at `2200` tokens by default through `MISTRAL_MAX_TOKENS` so the Context Carry block has enough room to reach the 1200-word target. The prompt now targets about 1200 useful words for substantial conversations, with explicit section budgets and instructions to preserve concrete files, functions, commands, errors, tests, deployment state, user constraints, unresolved risks, validation gaps, and uncertainty instead of a thin executive summary. The Mistral request timeout is 50 seconds and the extension backend timeout is 60 seconds so the larger model has room to finish instead of being cut off by the old speed-focused limits. `vercel.json` sets `api/summarize.js` `maxDuration` to 60 seconds. The endpoint returns timing metadata with the summary, including total backend time, Mistral time, model, max tokens, target words, input characters, and output characters. The extension/background trace also logs `inputChars`, `requestChars`, and `backendInputChars` so payload size can be checked from console timing.
