...
ChatGPT resolves one authoritative conversation scroll root from structural `conversation-turn-*` or author-role markers, with an app-sized largest-range fallback for detached virtualizers. Generic scrape candidates never vote on ChatGPT's scroll root, so surrounding chat UI cannot keep the real conversation pinned at the bottom. Other platforms retain the shared scroll-target path. Movement, step size, remaining distance, and diagnostics use the chosen geometry. The ChatGPT top reset currently logs the selected element and its scroll geometry immediately before and after the existing scroll call; the log does not alter capture behavior.
