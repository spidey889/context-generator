...
The summarizer uses a profile-selected path instead of one fixed model. Temperature is `0.1` for Mistral calls. `tiny` inputs up to 1,200 characters use a local direct Context Carry that preserves the exact source text without calling Mistral, because short chats do not need a generated summary. `small` and `medium` inputs use `ministral-3b-2512` for speed; only `large` inputs use `mistral-large-2512` for the high-quality 1,200-word handoff. Summary size is adaptive instead of one-size-fits-all: `tiny` targets about 120 words with zero output tokens because it is local; `small` inputs up to 8,000 characters target about 350 words with 1,000 output tokens and no expansion; `medium` inputs up to 60,000 characters target about 700 words with 1,900 output tokens and no expansion; only `large` inputs above 60,000 characters target about 1,200 words with 4,200 output tokens and the second-pass expansion safety net. The prompt and required template use the selected profile's section budgets so short chats do not duplicate themselves just to fill a giant 1,200-word template. If a large conversation produces a normalized summary under 1,100 words, the backend makes a second Mistral expansion pass using the original conversation and the draft summary, then uses the expanded version only if it is longer. The per-call Mistral request timeout is 80 seconds and the extension backend timeout is 190 seconds so the expansion pass has room to finish instead of being cut off by the old speed-focused limits. `vercel.json` sets `api/summarize.js` `maxDuration` to 180 seconds. The endpoint returns timing metadata with the summary, including total backend time, Mistral time, model, profile, max tokens, target words, minimum words, summary word count, Mistral pass count, expansion metadata, input characters, and output characters. The extension/background trace also logs `inputChars`, `requestChars`, and `backendInputChars` so payload size can be checked from console timing.
