...
The summarizer model is fixed to `mistral-large-2512`. Temperature is `0.1`, and output is capped at `3200` tokens by default through `MISTRAL_MAX_TOKENS` so the Context Carry block has enough room to reach the 1200-word target. The prompt and required template now target about 1200 useful words for substantial conversations, with explicit section budgets and instructions to preserve concrete files, functions, commands, errors, tests, deployment state, user constraints, unresolved risks, validation gaps, and uncertainty instead of a thin executive summary. The Mistral request timeout is 70 seconds and the extension backend timeout is 80 seconds so the larger model has room to finish instead of being cut off by the old speed-focused limits. `vercel.json` sets `api/summarize.js` `maxDuration` to 90 seconds. The endpoint returns timing metadata with the summary, including total backend time, Mistral time, model, max tokens, target words, input characters, and output characters. The extension/background trace also logs `inputChars`, `requestChars`, and `backendInputChars` so payload size can be checked from console timing.
...
- 2026-07-03: Live production probes after manual Vercel deploy confirmed the full 160k input reached Mistral, but the generated summary was still only about 763 words. The likely cause was the old template still asking for tiny `2-3 lines` sections. The template now contains section word budgets, default output rose to 3200 tokens, Mistral timeout rose to 70 seconds, extension backend timeout rose to 80 seconds, and Vercel maxDuration rose to 90 seconds.
