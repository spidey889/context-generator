...
The root `index.html` is a deliberately minimal, dependency-free marketing page with three parts: a centered hero and static handoff cue, a three-step explanation on a muted plum reading surface, and one control/privacy section with the final install action. Its restrained type scale, compact vertical rhythm, readable supporting text, and low-glare surfaces work across desktop and mobile without changing the centered composition. The hero gives only its closing display word a slow brand-purple shimmer. Navigation, buttons, and footer links retain at least 44-pixel touch targets, expose clear keyboard focus, and identify external store links to assistive technology. It has no JavaScript, build step, runtime API dependency, carousel, FAQ, animated demo, repeated proof metrics, or repeated install walkthrough. It uses relative assets and disables the shimmer for reduced-motion users. Its public install path is the Chrome Web Store for Chrome and Brave. The static handoff cue is a labeled illustration only and never captures a conversation or starts a transfer.
...
The handoff interaction is one continuous motion sequence: the floating button compresses on press, destination cards acknowledge selection immediately, the picker eases away while its backdrop is retained, and the handoff scrim and panel enter before that backdrop finishes fading. Handoff brand, status, and progress content then settle in with a short stagger; reduced-motion users receive the same states without animation.
...
The model must search the complete transcript before using `None`. `WHAT WE WERE DOING`, `WHERE WE LEFT OFF`, and `KEY CONTEXT` must remain meaningful and transcript-grounded. When a user explicitly marks a set of facts for exact retention, every fact must survive the handoff, including competing options, numeric values and ranges, safety or integrity statements, and current implementation state. `DECISIONS MADE` contains only user-made or clearly user-confirmed decisions, including deferred choices and accepted tradeoffs. The latest user-confirmed state wins conflicts; an older state is retained only when still relevant and labeled as replaced, rejected, changed, or historical.
...
- `npm run gate`: release command combining fast deterministic tests, the slow capture pacing regression, and live evaluation. GitHub Actions runs it on Node 22 after `master` pushes, daily, and on demand, using the current Node 24-based checkout and setup action runtimes. The workflow token is limited to read-only repository contents.
