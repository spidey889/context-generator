...
Placement is platform-specific: Claude anchors beside voice controls, ChatGPT near its model selector at the page root, Gemini near `Flash`, Grok near its mode selector, and DeepSeek near attachment. Grok alone accepts an expanded composer up to 720px tall. During the staged reflow after a large paste, it retains the already-verified outer composer through temporary input/container geometry mismatch and observes Grok input/composer resizing so placement follows the controls before paint; the shared 260px composer-surface limit and mutation-only monitoring remain unchanged for every other platform. Bounded editor/composer fallbacks handle hydration or selector drift; they are not conversation-capture fallbacks. Extension-owned nudges are excluded from capture.
