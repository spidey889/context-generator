...
ChatGPT resolves one authoritative conversation scroll root by walking upward from a structural `conversation-turn-*` or author-role marker and selecting the nearest ancestor whose computed `overflow-y` is exactly `auto` or `scroll`. Large ancestors with `overflow-y: visible` are never treated as scrollers, regardless of `scrollHeight`; there is no size-based or generic-selector fallback in the ChatGPT root resolver. Other platforms retain the shared scroll-target path. ChatGPT movement, step size, remaining distance, and diagnostics use the chosen root's geometry. Current diagnostics log the selected root immediately before and after the top-reset call and log the full ancestor chain from one structural conversation turn through `<html>`. These logs do not alter capture behavior.
