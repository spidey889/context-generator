...
The backend conversation input is capped at 160,000 characters to favor the best single transfer quality. If the page text is longer than that, the script keeps the first 32,000 characters and the recent tail, with an omission marker in the middle.
...
When the destination picker opens, `platform-content.js` checks whether the click is opening or closing the sheet. For opening clicks, `scheduleWarmSummary()` immediately starts a silent warm summary before creating or animating the picker DOM. The warm result is keyed to a lightweight conversation fingerprint and no longer auto-deletes on a timer; it is cleared when reused, when the fingerprint changes, or when the warm request fails. The sheet-open click creates a transfer trace id used by the source page, service worker, and destination page so console logs can show the full timeline from click to paste.
...
`api/summarize.js` is a Vercel serverless function. It allows CORS from any origin, handles `OPTIONS`, only accepts `POST`, and uses `MISTRAL_API_KEY` for the primary Mistral request. `GROQ_API_KEY` enables the Groq fallback path.
...
The summarizer uses two separate decisions: summary profile and Mistral model. Temperature is `0.1` for Mistral calls. `tiny` inputs up to 1,200 characters use a local direct Context Carry that preserves the exact source text without calling Mistral, because short chats do not need a generated summary. Summary size is adaptive instead of one-size-fits-all: `tiny` targets about 120 words with zero output tokens because it is local; `small` inputs up to 8,000 characters target about 350 words with 1,000 output tokens and no expansion; `medium` inputs up to 60,000 characters target about 700 words with 1,900 output tokens and no expansion; only `large` inputs above 60,000 characters target about 1,200 words with 4,200 output tokens and the second-pass expansion safety net. The prompt and required template use the selected profile's section budgets so short chats do not duplicate themselves just to fill a giant 1,200-word template.

For Mistral calls, default model routing is based on the sent conversation length: conversations at or below 20,000 characters use `ministral-3b-2512`; conversations above 20,000 characters use `mistral-large-2512`. If `MISTRAL_MODEL` is explicitly set, that override wins and size-based routing is skipped. This model routing does not move the existing profile/evaluation thresholds, so a 20,001-character conversation still uses the `medium` profile and its 1,900-token/no-expansion settings while calling the stronger model. The backend logs the selected model and reason, and returns `modelReason`, `modelInputChars`, `modelThresholdChars`, and `modelOverride` in timing metadata so the extension receipt can show the char count and threshold that chose the primary Mistral model.
...
The Groq fallback uses `GROQ_API_KEY` and `llama-3.1-8b-instant`. It does not alter Mistral's 20,000-character model routing or the `MISTRAL_MODEL` override; those still control only the primary Mistral attempt. Timing metadata now includes `servedBy`/`provider`, `primaryModel`, `providerMs`, `mistralMs`, `groqMs`, `providerPasses`, and `fallback` details so receipts can tell whether Mistral or Groq produced the returned summary. If a large conversation produces a normalized summary under 1,100 words, the backend makes a second expansion pass using the provider that served the initial summary, then uses the expanded version only if it is longer. The per-call provider request timeout is 80 seconds and the extension backend timeout is 190 seconds so the expansion pass has room to finish instead of being cut off by the old speed-focused limits. `vercel.json` sets `api/summarize.js` `maxDuration` to 180 seconds. The endpoint returns timing metadata with the summary, including total backend time, provider time, model, model routing reason, served backend, profile, max tokens, target words, minimum words, summary word count, provider pass count, expansion metadata, input characters, and output characters. The extension/background trace also logs `inputChars`, `requestChars`, and `backendInputChars` so payload size can be checked from console timing.
...
- 2026-07-10: The destination picker was visually upgraded without changing transfer behavior: a wider graphite surface, stronger system-font hierarchy, exact helper copy under the title, larger platform tiles, restrained accent auras, clearer hover/focus states, a paste-only reassurance footer, and a CSS mobile bottom-sheet fallback. The content-script load ID was bumped so refreshed tabs replace the old compact sheet.
