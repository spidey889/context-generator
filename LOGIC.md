...
Capture accepts platform-specific message elements only when the author is verified by structural evidence such as `data-message-author-role`, `data-role`, Claude test ids, Gemini user/model elements, or equivalent role-bearing ancestors. Loose “you” or “me” guesses are not used. Known empty-screen prompts are excluded even when a site renders them inside a role-bearing message-shaped node; this includes Grok's `What's on your mind?` prompt, which does not count as a real turn for handoff startup or capture.
