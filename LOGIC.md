...
For Claude, the button sits inside the composer as a composer-relative overlay without moving Claude-owned controls. The script now tries to anchor Cap-Context to the right side of Claude's voice mode control when that control is detectable. If that slot is not safely available, it searches for a clear in-composer fallback slot that avoids Claude's attach, project/sidebar, model, mic, voice, send, and other native buttons. It does not reserve space by transforming Claude's native controls. On startup, the content script also restores any old `data-context-generator-original-transform` reservation left by a stale injected script.
...
For Claude, `updateFloatingButtonPosition()` calls the Claude-specific branch before the old shared placement path. It releases any reserved action slot, leaves Claude's own controls untouched, and keeps Cap-Context mounted on the composer surface with absolute coordinates. It anchors to the right side of the voice mode control when possible, then falls back to a collision-checked in-composer slot instead of using the old generic rightmost-action-row anchor.
...
Claude placement should stay Claude-only, composer-surface mounted, and absolute-positioned. Do not append Cap-Context to Claude's action row and do not transform or reserve space in Claude's native controls. The preferred anchor is the right side of Claude's voice mode button. If the voice mode button cannot be detected, or the space beside it would collide with mic, voice, send, attach, model, project/sidebar, or another Claude-owned control, use the collision-checked fallback search inside the composer.
...
The summarizer model is fixed to `ministral-3b-2512`. Temperature is `0.1`, and output is capped at `650` tokens by default through `MISTRAL_MAX_TOKENS` so long generations do not make the handoff feel stuck. The endpoint returns timing metadata with the summary, including total backend time, Mistral time, model, max tokens, input characters, and output characters.
