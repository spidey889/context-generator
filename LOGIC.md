...
- `npm run gate`: release command combining fast deterministic tests, the slow capture pacing regression, and live evaluation. GitHub Actions runs it on Node 22 after `master` pushes, daily, and on demand, using the current Node 24-based checkout and setup action runtimes.
