...
The destination picker is a compact dark-glass decision surface anchored to the Cap Context bubble. It uses a prominent serif title, a left-aligned Cap Context mark, four alternate AI destinations in a two-column grid, full-tile hover and keyboard focus feedback, the original input-box helper copy, and a one-column layout on narrow viewports. Opening it remains UI-only and does not capture chat content.

The handoff is presented as a dark glass transfer capsule: a restrained ambient aurora establishes the brand moment, the live status remains the primary focal point, and a quiet divider separates it from the three real pipeline stages. The motion is decorative only, uses transform and opacity, and is disabled for reduced-motion users.
...
Placement is intentionally platform-specific: Claude centered beside its voice controls, ChatGPT as a fixed bubble left of the model selector, Gemini left of Pro/Flash, Grok beside its mode selector, and DeepSeek near attachment controls. On Claude's empty `/new` and `/chat/...` composers, the orb sits 1px above the PNG's exact optical center for a slightly more upbeat stance while retaining the standard 4px button-box right margin; this leaves a measured 13px visible gap to the voice dropdown so the controls read as one group. Typed Claude states retain their existing placement for separate tuning. Add `?__cap_context_debug_placement=1` to emit deduplicated Claude composer, anchor, bubble, and alignment geometry. Claude also watches class and style changes inside its stable composer because Voice and Send can swap visibility without resizing or remounting the DOM; that refresh keeps the active control clear of the bubble. Keep placement fixes scoped to their platform.
