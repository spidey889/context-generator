...
For Claude, the button sits inside the composer as a composer-relative overlay aligned with the bottom button row. The script tries to anchor Cap-Context to the right side of Claude's voice mode control when that control is detectable, and otherwise uses the rightmost small composer control as the row anchor. Claude is the one platform-specific exception where the right-side composer controls may be shifted left by a wider Cap-Context lane so the bubble can sit inline after mic and voice mode with native-looking spacing. On startup, the content script also restores any old `data-context-generator-original-transform` reservation left by a stale injected script.
...
- 2026-07-02: Claude inline placement now uses a wider 72px lane and 16px post-anchor gap. This replaces the tighter generic slot that made Claude's mic/voice controls look pushed into each other.
