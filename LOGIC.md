...
- Mistral is skipped without `MISTRAL_API_KEY`. The chain is Mistral Medium 3.5 (55-second budget) then Ministral 3 3B (25-second budget). Mistral Large 3 is excluded because the production key receives HTTP 403 for it. Rate limits are model-specific, so HTTP 429 advances immediately instead of retrying the same model.
