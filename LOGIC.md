...
`api/summarize.js` is a Vercel serverless function. It reflects CORS only for `chrome-extension://` and `moz-extension://` origins and accepts only `POST` plus valid extension preflight `OPTIONS`. Requests with an extension Origin remain compatible with already-running workers from before the public `X-Cap-Context-Client` marker shipped. Firefox extension requests may suppress the Origin header or send `Origin: null`, so those originless requests require the marker instead; ordinary web origins and unmarked originless clients are rejected. The marker is intentionally public and is not authentication or a secret. Extension-origin filtering, server-side rate limiting, and strict validation provide the practical abuse boundary.
