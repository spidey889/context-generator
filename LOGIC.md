...
Placement is platform-specific: Claude anchors beside voice controls, ChatGPT near its model selector at the page root, Gemini immediately left of its in-composer `Pro` or `Flash` model selector, Grok near its mode selector, and DeepSeek near attachment. All platforms accept expanded composers up to 720px tall. During staged reflow after a large paste, Claude, Gemini, Grok, and DeepSeek retain their already-verified outer composer while observing input/composer resizing. ChatGPT preserves its distinct page-root fixed-position architecture: it retains only the last verified placement surface and observes that surface plus the input, without mounting into or reserving the composer. Platform-specific control scoring and placement calculations remain unchanged. Bounded editor/composer fallbacks handle hydration or selector drift; they are not conversation-capture fallbacks. Extension-owned nudges are excluded from capture.
