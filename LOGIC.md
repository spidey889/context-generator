...
The protected `users` table contains one row per browser or extension install after that install's first successful summary. Its identity-backed `user_no` is assigned in first-success order, and it stores only the install id, lifetime successful-summary count, current UTC-day successful-summary count, and the UTC date for that daily count. Insert and status-transition triggers increment these aggregates only when a transfer first becomes `succeeded`. The table has RLS enabled, is readable only by `service_role`, and contains no conversation content, account, name, email, route, character count, failure detail, or per-transfer activity.
