...
Claude and ChatGPT capture also handles collapsed `Pasted content` cards inside verified user turns. Claude card detection accepts its real accessible label prefix `Pasted Text` even when the label continues with paste and line-count metadata. The scraper opens each rendered card sequentially; when its detail panel virtualizes absolute-positioned `[data-index]` rows, it finds that panel's nearest scrollable ancestor, sweeps top-to-bottom with overlap, and merges the indexed rows in numeric order. It then closes the panel and attaches the complete payload to the containing canonical user-message boundary before normal turn selection and serialization continue. This is capture-only behavior; other platforms, summary routing, and the local-direct carry format are unchanged.
