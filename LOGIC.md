...
3. A valid transfer keeps the source AI visible and opens the destination inactive while capture continues. Its `Capturing chat`, `Summarizing`, and `Pasting into <destination>` states advance only from real pipeline marks. Capture progress uses existing sweep geometry with linear connector motion; summary activity moves toward its below-completion cap over 20 seconds, then waits for the real summary-done mark. Every destination, including ChatGPT and Grok, remains inactive while its content script pastes and verifies the summary. After verified paste, any remaining connector distance finishes continuously over one second using compositor-driven motion; the completed destination tick is painted at the endpoint and the destination activates immediately afterward. The neutral, high-contrast fixed 40-second countdown becomes `Almost done, don't cancel now` after expiry. In-stage motion never gates capture, summary, or paste work; only the final one-second success reveal intentionally waits for the completion cue.
...
7. Before reuse, a prepared tab is re-read and must still match the selected destination; a missing, navigated-away, or failed prepared tab gets exactly one fresh correct destination. Normal and recovery ChatGPT pastes retain the same post-paste stability verification while remaining inactive during the transfer. Final destination activation is deferred until the real paste-success completion cue finishes; the tab is revalidated immediately before that activation.
