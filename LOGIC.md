...
Placement is platform-specific: Claude anchors beside voice controls, ChatGPT immediately left of its live `Instant`, `Medium`, or `High` model selector at the page root, Gemini immediately left of its in-composer `Pro` or `Flash` model selector, Grok near its mode selector, and DeepSeek near attachment. All platforms accept expanded composers up to 720px tall. During staged reflow after a large paste, Claude, Gemini, Grok, and DeepSeek retain their already-verified outer composer while observing input/composer resizing. ChatGPT preserves its distinct page-root fixed-position architecture: it retains only the last verified placement surface and observes that surface plus the input, without mounting into or reserving the composer. Its primary model-selector lookup is bounded to the live input row, recognizes button, role, tabindex, labeled, titled, and nested-text control structures, and resolves nested model text to its compact enclosing control for vertical alignment; composer-scoped lookup is legacy fallback only. Bounded editor/composer fallbacks handle hydration or selector drift; they are not conversation-capture fallbacks. Temporary ChatGPT-only console diagnostics currently trace model-selector detection, placement-rectangle selection, fallback selection, and final fixed-position updates without changing placement behavior. Extension-owned nudges are excluded from capture.
