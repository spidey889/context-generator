...
For Mistral calls, default model routing is based on the sent conversation length: conversations at or below 20,000 characters use `ministral-3b-2512`; conversations above 20,000 characters use `mistral-large-2512`. If `MISTRAL_MODEL` is explicitly set, that override wins and size-based routing is skipped. This model routing does not move the existing profile/evaluation thresholds, so a 20,001-character conversation still uses the `medium` profile and its 1,900-token/no-expansion settings while calling the stronger model. The backend logs the selected model and reason, and returns `modelReason`, `modelInputChars`, `modelThresholdChars`, and `modelOverride` in timing metadata so the extension receipt can show the char count and threshold that chose the model. If a large conversation produces a normalized summary under 1,100 words, the backend makes a second Mistral expansion pass using the original conversation and the draft summary, then uses the expanded version only if it is longer. The per-call Mistral request timeout is 80 seconds and the extension backend timeout is 190 seconds so the expansion pass has room to finish instead of being cut off by the old speed-focused limits. `vercel.json` sets `api/summarize.js` `maxDuration` to 180 seconds. The endpoint returns timing metadata with the summary, including total backend time, Mistral time, model, model routing reason, profile, max tokens, target words, minimum words, summary word count, Mistral pass count, expansion metadata, input characters, and output characters. The extension/background trace also logs `inputChars`, `requestChars`, and `backendInputChars` so payload size can be checked from console timing.
