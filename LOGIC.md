...
The destination picker is a compact dark-glass decision surface anchored to the Cap Context bubble. It uses a prominent serif title, a left-aligned Cap Context mark, four alternate AI destinations in a two-column grid, full-tile hover and keyboard focus feedback, the original input-box helper copy, and a one-column layout on narrow viewports. A small three-wisp purple mist bridge visually connects the bubble to the open picker; it uses transform/opacity motion only and becomes static under reduced motion. Opening the picker remains UI-only and does not capture chat content.
