...
When no useful chat text exists, user-triggered transfers do one short DOM-aware retry window before showing the empty-chat error. This keeps fast pages immediate while avoiding false `Nothing to carry yet` errors on slower hydrated chats. If no useful chat text appears after that window, the transfer stops before opening/preparing a destination tab or calling the backend and shows a polished in-page error: `Nothing to carry yet`. The message nudges the user to send something first.
