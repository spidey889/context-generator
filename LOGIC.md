...
For ChatGPT, `updateFloatingButtonPosition()` calls the ChatGPT-specific placement branch before `findComposerActionButton()`. That branch releases any old reserved action slot, finds the bottom-right composer row's intelligence/model selector (`Instant`, `Medium`, or `High`), and mounts the Cap Context button as an inline sibling immediately to the left of that selector. Because the button becomes part of the native control row, observer ticks do not repeatedly recalculate an absolute position or shift ChatGPT's own controls. If the intelligence selector cannot be detected, it falls back to the older absolute composer-row placement.
