...
When the destination picker opens, `platform-content.js` checks whether the click is opening or closing the sheet. For opening clicks, `scheduleWarmSummary()` captures the currently rendered chat before creating or animating the picker DOM. Short non-virtualized chats immediately start a silent warm summary. Chats with eight or more rendered turns defer the backend request until the full top-scroll and virtualized sweep completes, because Brave receipts proved that partial picker summaries were discarded on 13 of 14 recent Claude sweeps and caused two provider calls for one transfer. The warm result is keyed to a lightweight conversation fingerprint and no longer auto-deletes on a timer; it is cleared when reused, when the fingerprint changes, or when the warm request fails. The sheet-open click creates a transfer trace id used by the source page, service worker, and destination page so console logs can show the full timeline from click to paste.
...
The Groq fallback uses `GROQ_API_KEY` and `llama-3.1-8b-instant`. A provider-wide Mistral 429 uses the primary model's bounded retry and then moves directly to Groq instead of repeating the same rate limit across every Mistral model. Timing metadata includes `servedBy`/`provider`, `primaryModel`, `providerMs`, `initialMs`, `mistralMs`, `groqMs`, `providerPasses`, `finishReason`, `qualityFloorMet`, and fallback details. If a large conversation produces a normalized summary under 1,100 words, the backend makes a second expansion pass using the provider that served the initial summary, then uses the expanded version only if it is longer. Mistral expansion now extends the exact first-pass message prefix, reuses a non-sensitive `prompt_cache_key`, and supplies the normalized draft as a predicted output so the provider can reuse prompt and unchanged output work. The per-call provider timeout remains 80 seconds; an aborted model advances immediately instead of repeating the same 80-second attempt. The extension backend timeout is 175 seconds and it repeats the whole Vercel call only when the first failure arrives inside a 10-second retry budget. `vercel.json` sets `api/summarize.js` `maxDuration` to 180 seconds. The endpoint returns detailed input/output and per-pass timing so latency and quality-floor failures remain inspectable.
