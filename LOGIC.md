...
7. Before reuse, a prepared tab is re-read and must still match the selected destination; a missing, navigated-away, or failed prepared tab gets exactly one fresh correct destination. Normal and recovery ChatGPT pastes share the same activation settling and post-paste stability verification. Final destination activation is deferred until the real paste-success completion cue finishes; the tab is revalidated immediately before that activation.
