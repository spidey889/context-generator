...
- Removed the two remaining Claude transition artifacts: SPA pathname changes now refresh route-specific vertical alignment after `/new` sends, and newly visible Mic/Send controls receive their reserved transform synchronously before the browser can paint them beneath the orb.
