...
- Removed the remaining Claude transition artifacts: SPA pathname changes now refresh route-specific vertical alignment after `/new` sends, the docked chat orb is optically aligned to the visible mic glyph rather than its lower hitbox center, and newly visible Mic/Send controls receive their reserved transform synchronously before the browser can paint them beneath the orb.
