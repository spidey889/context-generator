...
- Reworked `LOGIC.md` into a current-code operational guide for agents, covering evidence priority, runtime ownership, platform behavior, invariants, message/storage contracts, capture and summary behavior, common wrong assumptions, known risks, focused verification, and a definition of done. Corrected the stale provider chain in the extension README and recorded the exact passing deterministic/failing Brave verification state at the same time.
...
- Refined the Cap Context orb using measured PNG artwork bounds: empty `/new` keeps a restrained 1px optical lift and 13px visible gap from the voice dropdown, while every `/chat/...` composer now lifts the orb 21px above Claude's low native controls. Added opt-in geometry logs and focused placement coverage while leaving typed `/new` separately tuned.
