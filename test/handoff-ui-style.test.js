...
test("handoff connectors accept live fractional progress without gating pipeline state", () => {
  assert.match(source, /handoff-stage-connector[\s\S]*background:rgba\(255,255,255,0\.11\)/);
  assert.match(source, /handoff-stage-connector-fill[\s\S]*width:var\(--context-generator-stage-progress-position,0%\)[\s\S]*transition:width var\(--context-generator-stage-progress-duration,1\.35s\)/);
  assert.match(source, /handoff-stage-progress-head[\s\S]*left:var\(--context-generator-stage-progress-position,0%\)[\s\S]*width:4px/);
