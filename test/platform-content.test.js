...
test("Claude bubble fills the inline slot to the right of voice mode", () => {
  ...
  assert.equal(placement.left, 722);
...
test("Claude crowded row shifts only small voice-side controls", () => {
  ...
  assert.equal(placement.inlineShift, 112);
  ...
  assert.equal(hooks.getClaudeControlTargetOffset(shiftedControls[0], placement.inlineShift), -60);
  assert.equal(hooks.getClaudeControlTargetOffset(shiftedControls[1], placement.inlineShift), -60);
});
