...
test("Claude bubble fills the inline slot to the right of voice mode", () => {
  ...
  assert.equal(placement.left, 710);
...
test("Claude crowded row shifts only small voice-side controls", () => {
  ...
  assert.equal(placement.inlineShift, 100);
  ...
  assert.equal(hooks.getClaudeControlTargetOffset(shiftedControls[0], placement.inlineShift), -48);
  assert.equal(hooks.getClaudeControlTargetOffset(shiftedControls[1], placement.inlineShift), -48);
});
